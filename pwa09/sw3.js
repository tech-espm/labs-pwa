...
const nomeDoCache = "cache-do-exemplo-09-v2";
...
self.addEventListener("activate", function (e) {
	// Pede para o processo de ativação esperar acabar a limpeza
	// dos caches antigos.
	e.waitUntil(
		// Lista o nome de todos os caches do nosso domínio.
		caches.keys().then(function (nomesDeCachesExistentes) {
			return Promise.all(
				nomesDeCachesExistentes.map(function (nome) {
					// Cria uma promessa para cada nome de cache que for diferente
					// do nome atual (se pudessem existir dois ou mais caches válidos
					// no nosso domínio, simultaneamente, talvez fosse melhor utilizar
					// técnicas como indexOf(), startsWith() ou endWith(), em vez de ===).
					if (nome !== nomeDoCache)
